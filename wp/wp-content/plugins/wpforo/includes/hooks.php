...
add_filter( 'login_url', function( $login_url ) {
	if(
        ...
        && strpos( (string) wp_debug_backtrace_summary(), 'wp_auth_check_html, wp_login_url,' ) === false
    ) {
...
add_filter( 'wp_dropdown_pages', function( $output, $r ) {
	...
	if( $r['name'] === 'page_for_posts' || ( $r['name'] === 'page_on_front' && wpforo_get_shortcode_pageid( $pageids ) ) ) {
	...
	if( is_wpforo_page() && ! is_wpforo_shortcode_page() ) {
		...
		if( isset( WPF()->current_object ) ) {
			if( wpfval( WPF()->current_object, 'forumid' ) && ! wpfval( WPF()->current_object, 'topicid' ) ) {
				...
				$description    = $title . ' - ' . wpforo_phrase( 'Member Profile', false ) . ' &gt; ' . wpforo_phrase( ucfirst( (string) $template ), false ) . ' ' . wpforo_phrase( 'Page', false ) . '. ' . implode( ', ', $udata );
...
function wpf_report() {
	...
	$reporter    = '<a href="' . WPF()->current_user['profile_url'] . '">' . ( WPF()->current_user['display_name'] ? WPF()->current_user['display_name'] : urldecode( (string) WPF()->current_user['user_nicename'] ) ) . '</a>';
...
function wpforo_get_login_or_register_notice_text() {
	$popup_html = '';
	if( ! wpforo_is_bot() ) {
		$popup_html = sprintf( wpforo_phrase( 'Please %s or %s', false ), '<a href="' . wpforo_login_url() . '" rel="noindex,nofollow">' . wpforo_phrase( 'Login', false ) . '</a>', '<a href="' . wpforo_register_url() . '" rel="noindex,nofollow">' . wpforo_phrase( 'Register', false ) . '</a>' );
	}
	...
	if( ! WPF()->perm->forum_can( 'va' ) || ! WPF()->usergroup->can( 'caa' ) ) {
		wp_add_inline_script(
            ...
            $('#wpforo-wrap').on('click', '.attach_cant_view', function(){
               wpforo_notice_show(
                    '<p>" . addslashes( (string) ( is_user_logged_in() ? wpforo_phrase( 'You are not permitted to view this attachment', false ) : wpforo_get_login_or_register_notice_text() ) ) . "</p>'
               );
            });
        })"
		);
...
function wpforo_admin_enqueue() {
	...
	if( ! empty( $_GET['page'] ) && strpos( (string) $_GET['page'], 'wpforo' ) === 0 ) {
		...
		} elseif( preg_match( '#^wpforo-(?:\d+-)?settings$#iu', (string) $_GET['page'] ) && ! empty( $_GET['wpf_tab'] ) && $_GET['wpf_tab'] === 'styles' ) {
			wp_enqueue_style( 'wp-color-picker' );
						...
						<?php foreach( $usergroups as $usergroup ): ?>
                            ...
                            <label style="min-width: 20%; display: inline-block; padding-bottom: 5px;">
                                <input type="checkbox"
									...
									<?php checked( in_array( (int) $usergroup['groupid'], (array) $secondary_groupids, true ) ); ?>>&nbsp;
...
function wpforo_avatar( $avatar, $id_or_email, $size, $default, $alt ) {
	...
	if( $user && is_object( $user ) ) {
		if( $src = WPF()->member->get_avatar_url( $user->data->ID ) ) {
			$avatar = "<img alt='" . esc_attr( $alt ) . "' src='" . esc_url( (string) $src ) . "' class='avatar avatar-" . esc_attr( $size ) . " photo' height='" . esc_attr( $size ) . "' width='" . esc_attr( $size ) . "' />";
		}
	}
...
function wpforo_delete_attachment( $attach_post_id ) {
	...
	if( ! empty( $posts ) || is_array( $posts ) ) {
		foreach( $posts as $post ) {
			$body = preg_replace( '|<div[^><]*id=[\'\"]+wpfa-' . $attach_post_id . '[\'\"]+[^><]*>.+?</div>|is', '<div class="wpforo-attached-file wpfa-deleted">' . wpforo_phrase( 'Attachment removed', false ) . '</div>', (string) $post['body'] );
			if( $body ) {
...
add_filter( 'wpforo_body_text_filter', function( $text ) {
	if( apply_filters( 'wpforo_allow_replace_3asterisk', true ) ) {
		$text = preg_replace( '#\*{3}([^*]+?)\*{3}#', '<span style="color: red; font-weight: bold;">$1</span>', (string) $text );
	}
...
function wpforo_user_field_shortcode_to_value( $shortcode, $userid = null, $implode = true, $separator = ',' ) {
	...
	if( $shortcode && ($field = preg_replace( '#^\s*\[?\s*(?:user_|owner_)?(?:fields_)?([^\[\]]+?)\s*]?\s*$#iu', '$1', (string) $shortcode )) ) {
...
function wpforo_forum_field_shortcode_to_value( $shortcode, $forumid, $implode = true, $separator = ',' ) {
	...
	if( $forumid ){
        if( $shortcode && ($field = preg_replace( '#^\s*\[?\s*(?:forum_)?(?:fields_)?([^\[\]]+?)\s*]?\s*$#iu', '$1', (string) $shortcode )) ) {
            if( $field === 'link' ){
                $value = sprintf( '<a target="_blank" href="%1$s">%2$s</a>', esc_url( (string) _wpforo_forum( $forumid, 'url' ) ), sanitize_text_field( _wpforo_forum( $forumid, 'title' ) ) );
...
function wpforo_topic_field_shortcode_to_value( $shortcode, $topicid, $implode = true, $separator = ',' ) {
    ...
    if( $topicid ){
        if( $shortcode && ($field = preg_replace( '#^\s*\[?\s*(?:topic_)?(?:fields_)?([^\[\]]+?)\s*]?\s*$#iu', '$1', (string) $shortcode )) ) {
            if( $field === 'link' ){
	            $value = sprintf( '<a target="_blank" href="%1$s">%2$s</a>', esc_url( (string) _wpforo_topic( $topicid, 'url' ) ), sanitize_text_field( _wpforo_topic( $topicid, 'title' ) ) );
...
function wpforo_post_field_shortcode_to_value( $shortcode, $postid, $implode = true, $separator = ',' ) {
    ...
    if($postid ){
        if( $shortcode && ($field = preg_replace( '#^\s*\[?\s*(?:post_)?(?:fields_)?([^\[\]]+?)\s*]?\s*$#iu', '$1', (string) $shortcode )) ) {
            if( $field === 'link' ){
	            $value = sprintf( '<a target="_blank" href="%1$s">%2$s</a>', esc_url( (string) _wpforo_post( $postid, 'url' ) ), sanitize_text_field( _wpforo_post( $postid, 'title' ) ) );
	            $value = stripslashes( $value );
            }else{
...
//		'meta'   => [ 'target' => '_blank' ]
	...
	if( wpforo_current_user_is( 'admin' ) || WPF()->usergroup->can( 'mf' ) || WPF()->usergroup->can( 'ms' ) || WPF()->usergroup->can( 'vm' ) || WPF()->usergroup->can( 'mp' ) || WPF()->usergroup->can( 'aum' ) || WPF()->usergroup->can( 'vmg' ) || WPF()->usergroup->can( 'mth' ) ) {
		$args = [
			'id'    => 'wpf-community',
			'title' => __( 'Forum Dashboard', 'wpforo' ) . wpforo_wp_admin_bar_red_circle_number($admin_bar_numbers['all_count']),
	...
	if( WPF()->usergroup->can( 'aum' ) || wpforo_current_user_is( 'admin' ) ) {
		$args = [
			'id'     => 'wpf-moderation',
			'title'  => '&#128479;&nbsp;&nbsp;' . __( 'Moderation', 'wpforo' ) . wpforo_wp_admin_bar_red_circle_number($admin_bar_numbers['mod_count']),
	...
	if( WPF()->usergroup->can( 'vm' ) || wpforo_current_user_is( 'admin' ) ) {
		$args = [
			'id'     => 'wpf-members',
			'title'  => '&#128100;&nbsp;&nbsp;' . __( 'Members', 'wpforo' ) . wpforo_wp_admin_bar_red_circle_number(wpforo_get_memb_attention_count()),
	...
	if( wpforo_current_user_is( 'admin' ) || WPF()->usergroup->can( 'mf' ) || WPF()->usergroup->can( 'ms' ) || WPF()->usergroup->can( 'vm' ) || WPF()->usergroup->can( 'mp' ) || WPF()->usergroup->can( 'aum' ) || WPF()->usergroup->can( 'vmg' ) || WPF()->usergroup->can( 'mth' ) ) {
		...
		if( $boardids = WPF()->board->get_active_boardids() ){
			foreach( $boardids as $boardid ){
				...
				$args = [
					'id'     => $menuid,
					'title'  => '&#128489;&nbsp;&nbsp;' . $current['title'] . ' (' . strtok( $current['locale'], '_' ) . ')' . wpforo_wp_admin_bar_red_circle_number( $bds_bar_numbers[$boardid]['all_count'] ),
					'href'   => admin_url( 'admin.php?page=' . wpforo_prefix_slug( 'dashboard' ) ),
					'parent' => 'wpforo',
				];
				$wp_admin_bar->add_node( $args );
...
//                    'meta'   => [ 'target' => '_blank' ]
				...
				if( WPF()->usergroup->can( 'aum' ) || wpforo_current_user_is( 'admin' ) ) {
					$args = [
						...
						'title'  => '&#128479;&nbsp;&nbsp;' . __( 'Moderation', 'wpforo' ) . wpforo_wp_admin_bar_red_circle_number( $bds_bar_numbers[$boardid]['mod_count'] ),
						'href'   => admin_url( 'admin.php?page=' . wpforo_prefix_slug( 'moderations' ) ),
						'parent' => $menuid,
					];
					$wp_admin_bar->add_node( $args );
        ...
        <div class="wpforo-rcn-wrap">
            <div class="wpforo-rcn-body">
				...
				<?php printf(
					...
					'<a class="wpf-rcnl" href="' . admin_url( 'admin.php?page=' . wpforo_prefix_slug( 'base-settings' ) . '&wpf_tab=recaptcha#wpf-settings-tab' ) . '" target="_blank">' . wpforo_phrase( 'configure', false, 'lower' ) . '</a>',
					'<a class="wpf-rcngl" href="https://developers.google.com/recaptcha" target="_blank">Google reCAPTCHA</a>',
					'<a href="' . admin_url( 'admin.php?page=' . wpforo_prefix_slug( 'base-settings' ) . '&wpf_tab=recaptcha#wpf-settings-tab' ) . '">' . wpforo_phrase( 'Tools &gt; Antispam Tab', false ) . '</a>',
