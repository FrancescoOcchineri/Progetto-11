				...
				if( isset( $item->url ) ) {
					if( strpos( (string) $item->url, '%wpforo-' ) !== false ) {
						...
						if( isset( WPF()->tpl->menu ) && isset( WPF()->tpl->menu[ $shortcode ] ) ) {
							...
							if (
								wpfval( WPF()->tpl->menu[ $shortcode ], 'is_active' )
								|| (
									isset( WPF()->tpl->menu[ $shortcode ]['attr'] )
									 && strpos( (string) WPF()->tpl->menu[ $shortcode ]['attr'], 'wpforo-active' ) !== false
								)
							) {
...
function wpforo_member_title( $member = [], $echo = true, $before = '', $after = '', $exclude = [] ) {
    ...
    if( ! in_array( 'custom-fields', $exclude, true ) ) {
        ...
        if( !empty( $fields ) ){
            ...
            if( $count ) $title[] = apply_filters( 'wpforo_member_custom_field_separator', '<span class="wpf-member-cf-sep">&nbsp;&nbsp;<i class="fas fa-bars"></i>&nbsp;</span>', $member );
            foreach ( $fields as $field ) {
                $title[] = '<span class="wpf-member-title wpf-member-cf wpf-member-cf-' . $field['name'] . '  wpfct" title="' . wpforo_phrase(
                        $field['label'],
                        false
                    ) . '">' . $field['value'] . ($count !== $i ? $after : '') . '</span>'; $i++;
            }
        }
        $before = $after = '';
    }
...
function wpforo_member_link( $user, $prefix = '', $size = 30, $class = '', $echo = true, $content = '', $attr = '' ) {
    ...
    if( $content === 'avatar' ){
	    ...
	    $ret = ( strpos( (string) $prefix, '%s' ) !== false ? sprintf( wpforo_phrase( $prefix, false ), esc_html( wpforo_text( $dname, $size, false ) ) ) : ( $prefix ? wpforo_phrase( $prefix, false ) . ' ' : '' ) . esc_html( wpforo_text( $dname, $size, false ) ) );
    }
...
function wpforo_topic_types( $topic, $class = true, $echo = true ) {
    ...
    $types = $class ? ( !empty( $topic_types ) ? trim( 'wpf-tt-' . implode( ' wpf-tt-', $topic_types ), ',') : '' ) : $topic_types;
    if( $echo ) echo $types;
    return $types;
}
			...
			if( $wrap ) {
                ...
                <div class="wpforo-post wpforo-tags wpfcl-1">
                    ...
                    <div class="<?php if( $type != 'text' ) echo 'wpf-tags'; ?> wpf-tags-<?php echo esc_attr(
						...
						<?php if( $type == 'text' ): ?>
	                        <i class="fas fa-tag"></i> <span class="wpf-ttt"><?php wpforo_phrase( 'Topic Tags' ); ?>:&nbsp; <?php endif; ?></span>
						<?php foreach( $tags as $tag ): ?>
	                        ...
	                        <tag wpf-tooltip="<?php echo esc_attr( wpforo_phrase( 'Topic Tag', false ) ); ?>">
			                    ...
			                    <?php if(is_rtl()): ?>
			                        <?php if( $count && wpfval($item,'count') && ! $topic['status'] ) echo '(' . $item['count'] . ') '; ?>
...
function wpforo_thread( $topicid ){
    ...
    if(!empty($thread['user']) || !empty($thread['last_user'])) {
        ...
        if( $thread['usergroup_can_va'] && $thread['feature_avatars'] ) {
            if( !empty( $thread['user'] ) ) {
                ...
                $thread['users_html'] .= '<div class="wpf-circle wpf-m">
                        <a href="' . esc_url( (string) $thread['url'] ) . '" wpf-tooltip="' . $thread['user_info'] . '" wpf-tooltip-position="top" wpf-tooltip-size="medium">' . $thread['user_avatar'] . '</a>
                    </div>';
            }
            if( $thread['replies'] && !empty( $thread['last_user'] ) ) {
                ...
                $thread['users_html'] .= '<div class="wpf-circle wpf-s">
                        <a href="' . esc_url( (string) $thread['last_post_url'] ) . '" wpf-tooltip="' . $thread['reply_user_info'] . '" wpf-tooltip-position="top" wpf-tooltip-size="medium">' . $thread['last_user_avatar'] . '</a>
                    </div>';
            }
        } else {
        ...
        if( $thread['usergroup_can_va'] && $thread['feature_avatars'] ) {
            if( !empty( $thread['user'] ) ) {
                $thread['user_avatar'] = wpforo_user_avatar($thread['user'], 40);
                $thread['author_html'] .= '<a href="' . esc_url( (string) $thread['url'] ) . '" wpf-tooltip="' . esc_attr(wpfval($thread['user'], 'display_name')) . '" wpf-tooltip-position="top" wpf-tooltip-size="medium">' . $thread['user_avatar'] . '</a>';
            }
        } else {
        ...
        if( $thread['usergroup_can_va'] && $thread['feature_avatars'] ) {
            if( $thread['replies'] && !empty( $thread['last_user'] ) ) {
                ...
                $thread['reply_html'] = '<i class="fas fa-reply fa-rotate-180"></i> <a href="' . esc_url( (string) $thread['last_post_url'] ) . '" wpf-tooltip="' . $thread['reply_user_info'] . '" wpf-tooltip-position="top" wpf-tooltip-size="medium">' . $thread['last_user_avatar'] . '</a>';
            } else {
                $thread['user_avatar'] = wpforo_user_avatar($thread['user'], 40);
                $thread['reply_html'] = '<i class="fas fa-feather-alt"></i> <a href="' . esc_url( (string) $thread['url'] ) . '" wpf-tooltip="' . esc_attr(wpfval($thread['user'], 'display_name')) . '" wpf-tooltip-position="top" wpf-tooltip-size="medium">' . $thread['user_avatar'] . '</a>';
            }
        }
...
function wpforo_sanitize_search_body( $body, $needle, $post = [] ) {
	...
	if( ! empty( $words ) ) {
		...
		if( strlen( (string) $body ) > $body_len && $pos !== false ) {
			...
			$body = $bef_body . mb_substr( (string) $body, $start, $body_len, get_option( 'blog_charset' ) ) . $aft_body;
		}
...
function wpforo_topic_title( $topic, $url, $structure = '{i}{au}{t}{/a}{n}{v}', $echo = true, $class = '', $length = 70 ) {
	...
	if( $title = trim( esc_html( (string) wpfval( $topic, 'title' ) ) ) ) {
		...
		$new         = ( strpos( (string) $structure, '{n}' ) !== false ) ? wpforo_unread_button(
			$topic['topicid'],
			$url,
			false
		) : '';
		$au          = ( strpos( (string) $structure, '{au}' ) !== false ) ? '<a href="' . wpforo_unread_url(
				$topic['topicid'],
				$url,
				false
			...
			if( ! empty( $best_answer ) ) {
				$schema .= ',
                    "acceptedAnswer": {
                        ...
                        "dateCreated": "' . gmdate( 'Y-m-d', strtotime( $best_answer['created'] . ' GMT' ) ) . 'T' . gmdate(
						'H:i',
					...
					$answers .= '
                        {
                            ...
                            "dateCreated": "' . gmdate( 'Y-m-d', strtotime( $suggested_answer['created'] . ' GMT' ) ) . 'T' . gmdate(
							'H:i',
...
function wpforo_profile_head_attrs( $user ){
    printf(
        ...
        esc_url( trim((string) wpfval( $user, 'cover' )) ? trim((string) wpfval( $user, 'cover' )) . '?lm=' . time() : wpforo_setting( 'profiles', 'default_cover' ) )
    );
}
