...
class Topics {
	...
	public function add( $args = [] ) {
		...
		$has_attach = ( isset( $has_attach ) && $has_attach ) ? 1 : ( ( strpos( (string) $body, '[attach]' ) !== false ) ? 1 : 0 );
	...
	public function edit( $args = [] ) {
		...
		$has_attach = ( isset( $body ) ? ( strpos( (string) $body, '[attach]' ) !== false ? 1 : 0 ) : $topic['has_attach'] );
        ...
        $body = ( !$min || ( isset( $body ) && $body ) ) ? stripslashes( (string) $body ) : stripslashes( (string) $post['body'] );
		...
		if( $cache ) {
			...
			if( ! empty( $object_cache ) ) {
				if( ! empty( $object_cache['items'] ) ) {
					...
					if( $access_filter ) {
						$object_cache['items'] = $this->access_filter( $object_cache['items'], [ 'groupids' => ( array_filter( array_map( 'intval', (array) $permgroup ) ) ?: null )] );
		...
		if( $needle !== '' ) {
			...
			foreach( $fields as $field ) {
				...
				if( $field === 'body' ) {
                    ...
                    $words = array_filter( explode(' ', sanitize_text_field( $needle ) ), function($word){ return mb_strlen((string) $word) > 2; });
					if( $words ){
						if( apply_filters( 'wpforo_suggested_topics_search_fields_method_regexp', false ) ){
							$words = array_map( function( $word ){ return str_replace(['"', "'"], ['\"', "\'"], preg_quote( preg_quote( $word ) ) ); }, $words );
	...
	public function sanitize_tag( $tag ) {
		...
		$length  = wpforo_setting( 'tags', 'length' );
		$tag     = ( function_exists( 'mb_substr' ) ) ? mb_substr( (string) $tag, 0, $length ) : substr( (string) $tag, 0, $length );
		$lowcase = wpforo_setting( 'tags', 'lowercase' );
		if( $lowcase ) {
			$tag = ( function_exists( 'mb_strtolower' ) ) ? mb_strtolower( (string) $tag ) : strtolower( (string) $tag );
		}
