...
class Forums {
	...
	public function rebuild_last_infos( $forumid, $update_parent_forums = true ) {
		...
		$sql = "SELECT `postid`, `created` FROM `" . WPF()->tables->posts . "` WHERE `status` = 0 AND `private` = 0 AND `forumid` IN(" . esc_sql( implode( ',', $forumids ) ) . ") ORDER BY `created` DESC, `postid` DESC LIMIT 1";
	...
	function forum_list( $forumids, $type = 'select_box', $selected = [], $cats = true, $disabled = [] ) {
		...
		foreach( $forumids as $forumid ) {
			...
			if(
				! $forumid
				|| !( WPF()->usergroup->can_manage_forum() || WPF()->perm->forum_can( 'vf', $forumid ) )
				|| ( $type === 'drag_menu' && ! WPF()->usergroup->can_manage_forum() )
			) continue;
            ...
            if( $type === 'select_box' ) { ?>
                        ...
                        <dt class="menu-item-handle forum_width">
                            ...
                            <span class="item-controls">
                                ...
                                <span class="menu_copy">
                                    <a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=' . wpforo_prefix_slug( 'forums' ) . '&wpfaction=forum_copy&forumid=' . $forumid ), 'forum_copy', '_wpfnonce' ) ?>" onclick="return confirm( '<?php wpforo_phrase( 'Are you sure you want to copy this forum?' ) ?>' )">
                ...
                <li>
					...
					<?php endif; ?>
                    <div class="wpf-sbs-div wpf-sbs-form-title<?php echo ( $depth > 0 ) ? ' wpf-sbs-forum' : ' wpf-sbs-cat'; ?>"><?php echo esc_html( str_repeat( '— ', $depth ) ) . trim( (string) $name ) ?></div>
                </li>
				<?php
			}
					...
					if( $selected ) {
						if( $type === 'select_box' ) {
							...
							foreach( $selected as $forumid => $stype ) {
								$html = preg_replace( '#(name=[\'"]wpforo\[forums\]\[' . intval( $forumid ) . '\][\'"][^<>]*?value=[\'"]' . preg_quote( $stype ) . '[\'"]|value=[\'"]' . preg_quote( $stype ) . '[\'"][^<>]*?name=[\'"]wpforo\[forums\]\[' . intval( $forumid ) . '\][\'"])#isu', '$1 checked', (string) $html );
							}
						}
					}
					echo $html;
	...
	public function get_forumid_and_childids( int $forumid, bool $check_permission = true ): array {
		...
		if( $check_permission ) $forumids = array_filter( $forumids, function( $forumid ){
